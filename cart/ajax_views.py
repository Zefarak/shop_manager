...
def ajax_order_item(request, action, pk):
    order_item = get_object_or_404(CartItem, id=pk)
    if action == 'add':
        order_item.qty += 1
    if action == 'remove':
    ...
    if action == 'delete':
        order_item.delete()
    instance = order_item.cart
    instance.refresh_from_db()
    data = dict()
    data['order_container'] = render_to_string(template_name='cart/include/order_container.html',
                                               request=request,
                                               context={
                                                   'instance': instance
                                               })
...
def ajax_add_product(request, pk, dk):
    order = get_object_or_404(Cart, id=pk)
    product = get_object_or_404(Product, id=dk)
    order_item, created = CartItem.objects.get_or_create(cart=order, title=product)
    if created:
